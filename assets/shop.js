...
function productCard(product, cart) {
  ...
  return `<article class="shop-product" data-category="${esc(product.category)}">
    <div class="shop-product__visual" role="img" aria-label="${esc(product.primary_image_alt)}" style="${imageStyle(product)}"></div>
    <div class="shop-product__body">
      ...
      <button class="shop-add" type="button" data-shop-add="${esc(variant?.id || '')}" ${!variant || !stock ? 'disabled' : ''}>${inCart ? `購物車已有 ${inCart} 件` : '加入購物車'}</button>
    </div>
  </article>`;
}
...
function shopShell(root) {
  root.innerHTML = `<div class="wrap shop-page"><section class="shop-hero"><div class="shop-hero__copy"><span class="sheet__eyebrow">BOOUNDLESS SHOP</span><h1>適合你的車，<br>保養用品一次買齊。</h1><p>從機油、濾芯到日常護理用品，清楚列出規格、價格與庫存。登入後即可儲存購物車及追蹤訂單。</p></div><div class="shop-hero__art" role="img" aria-label="機油、機油隔、冷卻液與汽車護理用品"></div></section><div class="shop-toolbar"><div><h2>保養用品</h2><p>購買前請以車主手冊指定規格為準。</p></div><div class="shop-categories" data-shop-categories></div></div><div class="shop-layout"><div class="shop-grid" data-shop-products><div class="shop-loading">正在載入商品…</div></div><aside class="shop-cart"><div class="shop-cart__head"><h2>購物車</h2><span class="shop-count" data-shop-count>0</span></div><div class="shop-cart__body" data-shop-cart></div></aside></div></div>`;
}
...
export async function renderShop(root, context = {}) {
  ...
  const draw = () => {
    const visible = activeCategory === '全部' ? products : products.filter((item) => item.category === activeCategory);
    productsHost.innerHTML = visible.length ? visible.map((item) => productCard(item, cart)).join('') : '<div class="shop-loading">這個分類暫時沒有商品。</div>';
  ...
  try {
    const productData = await api('/api/shop/products'); products = productData.products || [];
